...
</details>`};async function a(i){let{id:a=`58af05dfb6b4300151760176d2aad0a04c275aaadbb1315039263f021f920dcd`}=i.req.param(),o=i.req.query(`limit`)?Number.parseInt(i.req.query(`limit`),10):15,s=`https://www.chinaisa.org.cn`,c=new URL(`gxportal/xfpt/portal/getColumnList`,s).href,l=new URL(`gxportal/xfpt/portal/viewArticleById`,s).href,u=new URL(`gxportal/xfgl/portal/list.html?columnId=${a}`,s).href,d=await n(c,{method:`POST`,headers:{"Content-Type":`application/x-www-form-urlencoded`},body:new URLSearchParams({params:encodeURI(`{"columnId":"${a}"}`)}),parseResponse:JSON.parse}),f=r(d.articleListHtml),p=f(`ul.list li a`).slice(0,o).toArray().map(e=>(e=f(e),{title:e.prop(`title`)??e.text(),link:new URL(`gxportal/xfgl/portal/${e.prop(`href`)}`,s).href,guid:e.prop(`href`).match(/articleId=(\w+)/)[1],pubDate:t(e.parent().find(`span.times`).text().replaceAll(`[]`,``))}));p=await Promise.all(p.map(i=>e.tryGet(i.link,async()=>{let e=await n(l,{method:`POST`,headers:{"Content-Type":`application/x-www-form-urlencoded`},body:new URLSearchParams({params:encodeURI(`{"articleId":"${i.guid}","columnId":"${a}"}`)}),parseResponse:JSON.parse}),o=e.article_content,s=r(o),c=o.match(/文章来源：(.*?)日期：(\d+-\d+-\d+)/);return i.title=s(`div.article_title`).contents().first().text()||i.title,i.description=s(`div.article_main`).html(),i.author=c[1].split(/&/)[0],i.guid=`chinaisa-${i.guid}`,i.pubDate=t(c[2]),i})));let m=f(`div.head-tit`).text(),h=await n(u);f=r(h);let g=new URL(f(`link[rel="shortcut icon"]`).prop(`href`),s).href;return{item:p,title:`${f(`title`).text()} - ${m}`,link:u,description:f(`meta[name="description"]`).prop(`content`),language:`cn`,image:new URL(`img/logo.jpg`,s).href,icon:g,logo:g,subtitle:m,allowEmpty:!0}}export{i as route};
//# sourceMappingURL=chinaisa-24bo_Omk.js.map