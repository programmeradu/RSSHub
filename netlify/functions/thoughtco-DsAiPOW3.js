...
</details>`};async function l(t){let{category:c=``}=t.req.param(),l=t.req.query(`limit`)?Number.parseInt(t.req.query(`limit`),10):50,u=`https://www.thoughtco.com`,d=new URL(c,u).href,{data:f}=await a(d),p=s(f),m=p(`a[data-doc-id]`).slice(0,l).toArray().map(e=>(e=p(e),{title:e.find(`span.block-title`).text(),link:new URL(e.prop(`href`),u).href}));m=await Promise.all(m.map(t=>n.tryGet(t.link,async()=>{let{data:n}=await a(t.link),c=s(n);return c(`div.adslot`).remove(),c(`div.sources-and-citation, .mntl-figure-caption svg`).remove(),c(`div.figure-media`).each((t,n)=>{n=p(n);let i=n.find(`img`);n.replaceWith(r(o.join(e,`templates/description-0631c1c3.art`),{image:{src:i.prop(`data-src`),width:i.prop(`width`),height:i.prop(`height`)}}))}),t.title=c(`meta[property="og:title"]`).prop(`content`),t.description=r(o.join(e,`templates/description-0631c1c3.art`),{image:{src:c(`meta[property="og:image"]`).prop(`content`)},description:c(`div.article-content`).html()}),t.author=c(`meta[name="sailthru.author"]`).prop(`content`),t.category=[...new Set(c(`meta[name="parsely-tags"]`).prop(`content`)?.split(/,/).map(e=>e.trim()))],t.pubDate=i(n.match(/"datePublished": "(.*?)"/)[1]),t.updated=i(n.match(/"dateModified": "(.*?)"/)[1]),t})));let h=p(`meta[property="og:site_name"]`).prop(`content`),g=p(`meta[property="og:title"]`).prop(`content`),_=new URL(p(`link[rel="apple-touch-icon-precomposed"]`).prop(`href`),u).href;return{item:m,title:`${h}${g.startsWith(h)?``:` - ${g}`}`,link:d,description:p(`meta[property="og:description"]`).prop(`content`),language:p(`html`).prop(`lang`),image:p(`meta[property="og:image"]`).prop(`content`),icon:_,logo:_,subtitle:p(`meta[property="og:title"]`).prop(`content`),author:h}}export{c as route};
//# sourceMappingURL=thoughtco-DsAiPOW3.js.map